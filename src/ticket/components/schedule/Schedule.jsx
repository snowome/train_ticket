...
const ScheduleRow = memo(function ScheduleRow(props) {
    const {
        index, station, departTime, arriveTime, stay,
...
const Schedule = memo(function Schedule(props) {
    ...
    useEffect(() => {
        const url = new URI(`${PROXY_URL}/rest/schedule`)
                ...
                <ul>
                    {
                        scheduleList.map((item, index) => {
                            return <ScheduleRow index={index + 1} {...item} key={index}></ScheduleRow>
                        })
                    }
                </ul>
            </div>
