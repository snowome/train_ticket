...
import './Bottom.css'
import {ORDER_DEPART} from "../../config";
...
const Filter = memo(function Filter(props) {
    ...
    return (
        <li className={classnames({'checked': checked})} onClick={() => dispatch({type: 'toggle', payload: value})}>{name}</li>
    )
})
Filter.propTypes = {
            ...
            <ul>
                {
                    options.map((option, index) => {
                        return (
                            <Filter {...option} checked={option.value in checkedMap} key={index}
            ...
            <div className="bottom-dialog">
                <div className="bottom-dialog-content">
                    ...
                    <div className="options">
                        {
                            optionGroup.map((group, index) => <Option {...group} key={index}></Option>)
                        }
                        <Slider title="出发时间"
                                currentStartHours={localDepartTimeStart} currentEndHours={localDepartTimeEnd}
                                onStartChanged={setLocalDepartTimeStart} onEndChanged={setLocalDepartTimeEnd}
                        ></Slider>
                        <Slider title="到达时间"
                                currentStartHours={localArriveTimeStart} currentEndHours={localArriveTimeEnd}
                                onStartChanged={setLocalArriveTimeStart} onEndChanged={setLocalArriveTimeEnd}
                        ></Slider>
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="bottom">
            <div className="bottom-filters">
                ...
                <span className={classnames('item', {'item-on': onlyTickets})}
                      onClick={toggleOnlyTickets}>
                    <i className="icon">{onlyTickets ? '\uf43d' : '\uf43c'}</i>
                    只看有票
                </span>
                <span className={classnames('item', {'item-on': isFiltersVisible || !noChecked})}
            ...
            {isFiltersVisible && (
            <BottomModel
                    ticketTypes={ticketTypes} trainTypes={trainTypes} departStations={departStations}
                    ...
                    setCheckedTicketTypes={setCheckedTicketTypes} setCheckedTrainTypes={setCheckedTrainTypes}
                    setCheckedDepartStations={setCheckedDepartStations}
                    setCheckedArriveStations={setCheckedArriveStations}
                    setDepartTimeStart={setDepartTimeStart} setDepartTimeEnd={setDepartTimeEnd}
                    setArriveTimeStart={setArriveTimeStart} setArriveTimeEnd={setArriveTimeEnd}
                    toggleIsFiltersVisible={toggleIsFiltersVisible}></BottomModel>
            )}
        </div>
    )
}
